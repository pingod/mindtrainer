...
function createDitheredObject(elements, options = {}) {
  ...
  function updateDiffusion() {
    ...
    renderer.readRenderTargetPixelsAsync(surface, 0, 0, width, height, pixels).then(() => {
  ...
  function resize() {
    ...
    const pixelSize = config.dither ? Math.max(config.gridSize, 1) * Math.max(config.pixelSizeRatio, 1) * pr : 1;
