...
function mapShapeUvs(geometry, bounds) {
  let minX = 0;
  let minY = 0;
  let spanX;
  let spanY;
...
function createLiquidObject(elements, options = {}) {
  ...
  const baseLooks = /* @__PURE__ */ new Map();
  ...
  function onPointerMove(event) {
    ...
    const kick = Math.max(config.wobble, 0) * 25e-4;
  ...
  function splat(target, value, radius) {
  ...
  function applyOptions() {
    ...
    for (const [material, base] of baseLooks) {
      ...
      if (typeof standard.roughness === "number") {
        standard.roughness = base.roughness + (0.25 - base.roughness) * metallic;
      }
    }
    ...
    compositePass.uniforms.uLensRadius.value = 0.12 + Math.min(Math.max(config.cursorSize, 0.02), 1) * 0.45;
    ...
    compositePass.uniforms.uAmbient.value = reducedMotion ? 0 : Math.max(config.ambient, 0);
    ...
    if (config.background) {
      ...
      compositePass.uniforms.uBackground.value.set(
        backgroundColor.r,
        backgroundColor.g,
        backgroundColor.b
      );
  ...
  function tick(time) {
    ...
    const delta = lastTime ? Math.min((time - lastTime) / 1e3, 0.05) : 0;
    lastTime = time;
    if (envDirty) {
      envDirty = false;
      refreshEnvironment();
    ...
    if (delta > 0 && (queued.length > 0 || simEnergy > 2e-3)) {
    ...
    compositePass.uniforms.uTime.value = time * 1e-3;
